...
Moreover, even their classifications are subject to thresholds[^3]. For instance, if a prediction score exceeds 0.5, it might be categorized as class A; if below, class B. This uncertainty is built into the model’s architecture. When an LLM produces text, it is not stating facts. It is selecting the most statistically probable sequence of tokens based on your prompt and its internal weights[^4].
...
But outside of these stable domains, problems arise. When discussing complex philosophical arguments, subtle historical events, or niche knowledge areas, AI is more likely to hallucinate[^10]. Without a specific “guiding path,” such as a well-engineered prompt or a retrieval system that brings in factual context (RAG)[^9], the model may produce misleading or simply incorrect answers.
...
Importantly, educators should not focus solely on teaching prompt engineering. Instead, the emphasis should remain on **fact-checking**, **critical reasoning**, and the ability to distinguish between authority and probability[^11].
...
[^2]: [Hello World](../short/Hello%20World.md)

[^3]: [Thresholds and the confusion matrix  |  Machine Learning  |  Google for Developers](https://developers.google.com/machine-learning/crash-course/classification/thresholding)

[^4]: [Large language model - Wikipedia](https://en.wikipedia.org/wiki/Large_language_model#Prompt_engineering,_attention_mechanism,_and_context_window)

[^5]: [Cloud of Poems - Wikipedia](https://en.wikipedia.org/wiki/Cloud_of_Poems)

[^6]: [cuhk.edu.hk/rct/pdf/e_outputs/b7778/v7778p087.pdf](https://www.cuhk.edu.hk/rct/pdf/e_outputs/b7778/v7778p087.pdf#page=1.41)

[^7]: [A Reinterpretation of Confucian Philosophy of Education](https://www.scirp.org/journal/paperinformation?paperid=77724)

[^8]: [Socratic method - Wikipedia](https://en.wikipedia.org/wiki/Socratic_method)

[^9]: [Retrieval-augmented generation - Wikipedia](https://en.wikipedia.org/wiki/Retrieval-augmented_generation)
